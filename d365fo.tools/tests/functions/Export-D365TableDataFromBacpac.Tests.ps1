Describe "Export-D365TableDataFromBacpac Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Export-D365TableDataFromBacpac).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter Path' {
			$parameter = (Get-Command Export-D365TableDataFromBacpac).Parameters['Path']
		...
		It 'Should have the expected parameter TableName' {
			$parameter = (Get-Command Export-D365TableDataFromBacpac).Parameters['TableName']
		...
		It 'Should have the expected parameter OutputPath' {
			$parameter = (Get-Command Export-D365TableDataFromBacpac).Parameters['OutputPath']
	...
	Describe "Testing parameterset __AllParameterSets" {
		<#
		__AllParameterSets -Path -TableName
