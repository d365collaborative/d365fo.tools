Describe "Start-D365Environment Unit Tests" -Tag "Unit" {
	...
	Describe "Ensuring unchanged command signature" {
		...
		It 'Should have the expected parameter DocumentRouting' {
			$parameter = (Get-Command Start-D365Environment).Parameters['DocumentRouting']
	...
	Describe "Testing parameterset Default" {
 	...
 	Describe "Testing parameterset Specific" {
		<#
		Specific -
		Specific -ComputerName -Aos -Batch -FinancialReporter -DMF -DocumentRouting -OnlyStartTypeAutomatic -ShowOriginalProgress
		#>
	}
