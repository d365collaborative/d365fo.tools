Describe "New-D365CAReport Unit Tests" -Tag "Unit" {
	...
	Describe "Ensuring unchanged command signature" {
		...
		It 'Should have the expected parameter Path' {
			$parameter = (Get-Command New-D365CAReport).Parameters['Path']
	...
	Describe "Testing parameterset __AllParameterSets" {
		...
		__AllParameterSets -Path -Module -Model -BinDir -MetaDataDir -XmlLog -ShowOriginalProgress -OutputCommandOnly
		#>
	}
