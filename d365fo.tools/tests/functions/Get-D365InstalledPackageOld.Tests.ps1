Describe "Get-D365InstalledPackageOld Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-D365InstalledPackageOld).ParameterSets.Name | Should -Be 'Default'
		}
		
		It 'Should have the expected parameter Name' {
			$parameter = (Get-Command Get-D365InstalledPackageOld).Parameters['Name']
		...
		It 'Should have the expected parameter PackageDirectory' {
			$parameter = (Get-Command Get-D365InstalledPackageOld).Parameters['PackageDirectory']
