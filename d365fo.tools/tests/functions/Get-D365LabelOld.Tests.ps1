Describe "Get-D365LabelOld Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-D365LabelOld).ParameterSets.Name | Should -Be 'Default', 'Specific'
		}
		
		It 'Should have the expected parameter FilePath' {
			$parameter = (Get-Command Get-D365LabelOld).Parameters['FilePath']
		...
		It 'Should have the expected parameter Name' {
			$parameter = (Get-Command Get-D365LabelOld).Parameters['Name']
		...
		It 'Should have the expected parameter Value' {
			$parameter = (Get-Command Get-D365LabelOld).Parameters['Value']
		...
		It 'Should have the expected parameter IncludePath' {
			$parameter = (Get-Command Get-D365LabelOld).Parameters['IncludePath']
