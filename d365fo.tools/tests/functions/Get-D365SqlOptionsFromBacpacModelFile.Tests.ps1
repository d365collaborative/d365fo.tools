Describe "Get-D365SqlOptionsFromBacpacModelFile Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-D365SqlOptionsFromBacpacModelFile).ParameterSets.Name | Should -Be 'ImportTier1'
		}
		
		It 'Should have the expected parameter Path' {
			$parameter = (Get-Command Get-D365SqlOptionsFromBacpacModelFile).Parameters['Path']
