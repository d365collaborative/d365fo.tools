Describe "Set-D365ActiveEnvironmentConfig Unit Tests" -Tag "Unit" {
	...
	Describe "Ensuring unchanged command signature" {
		...
		It 'Should have the expected parameter ConfigStorageLocation' {
			$parameter = (Get-Command Set-D365ActiveEnvironmentConfig).Parameters['ConfigStorageLocation']
		...
		It 'Should have the expected parameter Temporary' {
			$parameter = (Get-Command Set-D365ActiveEnvironmentConfig).Parameters['Temporary']
