Describe "Get-D365AOTObjects Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-D365AOTObjects).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should habe the expected parameter Path' {
			$parameter = (Get-Command Get-D365AOTObjects).Parameters['Path']
		...
		It 'Should habe the expected parameter ObjectType' {
			$parameter = (Get-Command Get-D365AOTObjects).Parameters['ObjectType']
		...
		It 'Should habe the expected parameter Name' {
			$parameter = (Get-Command Get-D365AOTObjects).Parameters['Name']
		...
		It 'Should habe the expected parameter SearchInPackages' {
			$parameter = (Get-Command Get-D365AOTObjects).Parameters['SearchInPackages']
		...
		It 'Should habe the expected parameter IncludePath' {
			$parameter = (Get-Command Get-D365AOTObjects).Parameters['IncludePath']
