<#
.SYNOPSIS
Send message to online users in D365FO

.DESCRIPTION
Utilize the same messaging framework available from LCS and send a message to all online users in the environment

.PARAMETER AuthProviderUri
The URL / URI for the authentication provider that you want to authenticate against

Default is https://login.microsoftonline.com/[TENANTID]/oauth2/token

[TENANTID] is the GUID from your Azure Active Directory then D365FO is connected to
...
.EXAMPLE
PS C:\> Send-D365MessageToUsers -AuthProviderUri "https://login.microsoftonline.com/e674da86-7ee5-40a7-b777-1111111111111/oauth2/token" -URL "https://usnconeboxax1aos.cloud.onebox.dynamics.com" -ClientId "dea8d7a9-1602-4429-b138-111111111111" -ClientSecret "Vja/VmdxaLOPR+alkjfsadffelkjlfw234522="
...
Tags: Servicing, Message, Users, Environment, ClientId, ClientSecret

Author: Mötz Jensen (@Splaxi)

#>

function Send-D365MessageToUsers {
    [CmdletBinding()]
    [OutputType()]
    param (
    ...
    $bearerParms = @{
        AuthProviderUri = $AuthProviderUri
        Resource        = $URL
        ClientId        = $ClientId
